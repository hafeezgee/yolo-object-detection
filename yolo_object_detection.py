...
Original file is located at
    https://colab.research.google.com/drive/1FDY3pMYXrSJIbHGNr1Dccaemg5Ps-pIu
"""
...
def get_notebook_content():
    """
    Retrieves the content of the current Colab notebook as a JSON string.
    """
    # This JavaScript snippet gets the current notebook's content from the frontend.
    js_code = '''
        async function getNotebookContent() {
            const res = await fetch('/api/contents/' + Jupyter.notebook.path);
            const json = await res.json();
            return JSON.stringify(json.content);
        }
        getNotebookContent(); // Changed this line: removed 'return'
    '''
    notebook_json_str = eval_js(js_code)
    return notebook_json_str

# Get the content of the current notebook
print("Fetching current notebook content...")
current_notebook_content_json = get_notebook_content()

# Parse the JSON string into a notebook object
nb = nbf.reads(current_notebook_content_json, as_version=4)
...
output_filename = "clean_yolo_object_detection.ipynb" # Using the name from your previous attempt

# Write the cleaned notebook to a new file
with open(output_filename, 'w', encoding='utf8') as f:
    nbf.write(nb, f)

print(f"Clean notebook saved as '{output_filename}'!")

print("You can download this file from the Colab file browser (left sidebar) or use `files.download()`.")